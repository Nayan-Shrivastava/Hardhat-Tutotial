...
describe("ERC721 Token", () => {
  ...
  beforeEach(async () => {
    ...
    token = await Token.deploy("My Test NFT", "MTN");
    [owner, addr1, addr2, addr3, _] = await ethers.getSigners();
    zeroAddress = "0x0000000000000000000000000000000000000000";
  });

  describe("deployment", () => {
    it("should return right name and symbol", async () => {
      const tokenName = await token.name();
      expect(tokenName).to.equal("My Test NFT");

      const tokenSymbol = await token.symbol();
      expect(tokenSymbol).to.equal("MTN");
    });
  });

  describe("mint", async () => {
    it("should mint token to given account", async () => {
    ...
    it("should not mint if account address is zero address or token already exists", async () => {
      await expect(
